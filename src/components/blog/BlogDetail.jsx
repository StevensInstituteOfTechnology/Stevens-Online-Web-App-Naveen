        ...
        switch (item.type) {
          case "paragraph":
            return (
              <p
                key={index}
                className="mb-stevens-lg leading-relaxed text-stevens-lg"
              >
            ...
            return (
              <ListTag key={index} className={listClasses}>
                {item.items.map((listItem, itemIndex) => (
                  <li
                    key={itemIndex}
                    className="leading-relaxed text-stevens-lg"
                  >
...
const renderTextWithFormatting = (text, bold = [], links = []) => {
  ...
  if (links && links.length > 0) {
    links.forEach((link) => {
      ...
      processedText = processedText.replace(
        regex,
        `<a href="${actualUrl}" target="_blank" rel="noopener noreferrer" class="text-stevens-red hover:text-stevens-red underline">${link.text}</a>`,
      );
    });
  }
        ...
        <section className="border-t border-stevens-light-gray pt-stevens-2xl">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-stevens-lg">
            {relatedPosts.map((relatedPost) => (
              <div
                key={relatedPost.id}
                className="border border-stevens-light-gray rounded-stevens-md p-stevens-md hover:shadow-stevens-md transition-shadow"
              >
                <img
                  {...getThumbnailImageProps(
                  ...
                  style={{
                    objectPosition: relatedPost.image_position || "center",
                  }}
                  loading="lazy"
                />
